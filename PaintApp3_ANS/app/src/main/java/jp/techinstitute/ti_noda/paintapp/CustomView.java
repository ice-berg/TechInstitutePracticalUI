...
public class CustomView extends View {
	 ...
	 * @param eraseColor
	 */
	public void Undo(int eraseColor){
		// 元の色を保持しておく
		...
		// 消す色を設定する
		mPaint.setColor(eraseColor);
		// アンチエイリアスは不要なので無効にする
		mPaint.setAntiAlias(false);
		// ディザは不要なので有効にする
		mPaint.setDither(false);
		// 線幅を1pixelだけ広げておく(そのままだと誤差分が出るため)
		mPaint.setStrokeWidth(width + 1);

		// 1つ戻すために、mCanvasを使ってmBitmapに消す色でmPath(最後に描画した線情報)を書き込む
		mCanvas.drawPath(mPath, mPaint);
		// mBitmapに書き込み終わったので、リセットしておく
		// ※ここでリセットしないと再描画時(onDraw)にmPathの情報を描画してしまうため
		mPath.reset();
